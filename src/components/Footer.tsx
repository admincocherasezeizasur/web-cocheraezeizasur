        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-10">
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-3">
              <a href={siteConfig.contact.googleMapsUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-[13px] text-gray-500 hover:text-white transition-colors cursor-pointer">
                <Icon icon="mdi:map-marker-outline" className="w-4 h-4" />
                {siteConfig.contact.address}
              </a>
              <a href={`https://wa.me/${siteConfig.contact.whatsappNumber}?text=${encodeURIComponent(dict?.nav?.contacto_whatsappMessage || "Hola! Me contacto desde su página web, quiero más información.")}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-[13px] text-gray-500 hover:text-white transition-colors cursor-pointer">
                <Icon icon="mdi:phone-outline" className="w-4 h-4" />
                {siteConfig.contact.phoneDisplay}
              </a>
              <a href={`mailto:${siteConfig.contact.email}`} className="flex items-center gap-3 text-[13px] text-gray-500 hover:text-white transition-colors cursor-pointer">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center gap-10">
            <div className="flex gap-6">
              <Link href={`/${lang}/terminos`} className="text-[13px] text-gray-500 hover:text-white transition-colors">
                {dict?.footer?.legales?.[0] || "Términos y Condiciones"}
              </Link>
              <Link href={`/${lang}/politicas`} className="text-[13px] text-gray-500 hover:text-white transition-colors">
